...
__inline_double zeta(double zz, double zl) { return ((zz - zl) * (re + zl) / (re + zz)); }

double densm(double alt, double d0, double xm, double *tz, int mn3, double *zn3, double *tn3, double *tgn3, int mn2,
...
double densu(double alt, double dlb, double tinf, double tlb, double xm, double alpha, double *tz, double zlb,
             double s2, int mn1, double *zn1, double *tn1, double *tgn1)
{
...
/*    Eq. A24c */
__inline_double sumex(double ex) { return (1.0 + (1.0 - pow(ex, 19.0)) / (1.0 - ex) * pow(ex, 0.5)); }
 ...
 */
    ...
    {
        t[10] = (1.0 +
                 plg[0][1] * (p[80] * flags->swc[5] * cos(dr * (input->doy - p[81])) +
 ...
 */
    ...
    meso_tgn2[1] = pavgm[8] * pma[9][0] * (1.0 + flags->sw[20] * flags->sw[22] * glob7s(pma[9], input, flags)) *
 ...
 */
        ...
        meso_tgn3[1] = pma[7][0] * pavgm[7] * (1.0 + flags->sw[22] * glob7s(pma[7], input, flags)) * meso_tn3[4] *
                       meso_tn3[4] / (pow((pma[6][0] * pavgm[6]), 2.0));
    }
...
void gtd7d(struct nrlmsise_input *input, struct nrlmsise_flags *flags, struct nrlmsise_output *output)
{
    ...
    output->d[5] = 1.66E-24 * (4.0 * output->d[0] + 16.0 * output->d[1] + 28.0 * output->d[2] + 32.0 * output->d[3] +
                               40.0 * output->d[4] + output->d[6] + 14.0 * output->d[7] + 16.0 * output->d[8]);
    if (flags->sw[0])
        output->d[5] = output->d[5] / 1000;
}
 ...
 *       density above 300 km */
    if (input->alt < 300.0)
    {
        ...
        meso_tgn1[1] = ptm[8] * pma[8][0] * (1.0 + flags->sw[18] * flags->sw[20] * glob7s(pma[8], input, flags)) *
