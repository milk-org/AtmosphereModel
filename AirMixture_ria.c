...
)
{
    ...
    for(int specindex = 0; specindex < atmNBspecies; specindex++)
    {
        ...
        if(speciesRIA.RIA_species[specindex].init == 1)
        {
            ...
            float alpha = (lambda - speciesRIA.RIA_species[specindex].lambda[lli]) /
                          (speciesRIA.RIA_species[specindex].lambda[lli + 1] -
                           speciesRIA.RIA_species[specindex].lambda[lli]);
            //    printf("%d   alpha = %f \n", llidir, alpha);
            lliprecomp[specindex] = lli;
            n = (1.0 - alpha) * speciesRIA.RIA_species[specindex].rindex[lli] + alpha *
                speciesRIA.RIA_species[specindex].rindex[lli + 1];
        ...
        double tmpc = densarray[specindex] / (LoschmidtConstant / 1e6 /
                          speciesRIA.RIA_species[specindex].Z);
        LL *= tmpc;
