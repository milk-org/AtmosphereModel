...
int AtmosphereModel_build_stdAtmModel(ATMOSPHERE_MODEL *atm, const char *restrict fname)
{
    FILE *fp;
    ...
    for (long zindex = 0; zindex < ATM_VPROF_NBSTEP; zindex++)
    {
        ...
        input[zindex].sec = (int)(3600.0 * (atm->TimeLocalSolarTime - atm->SiteLong / 15.0));
...

    {
        ...
        atm->vprof_dens_species[speciesNe].val[zindex] = output[zindex].d[2] * 2.328e-5;

        DEBUG_TRACEPOINT("compute total density");

        atm->vprof_denstot.val[zindex] = output[zindex].d[0] + output[zindex].d[1] + output[zindex].d[2] +
        ...
        atm->vprof_pressure.val[zindex] =
            atm->vprof_denstot.val[zindex] * 1.0e6 / LoschmidtConstant * (output[zindex].t[1] / 273.15);
        printf("Temperature at site altitude (%12f m) = %8f K\n", atm->SiteAlt, output[zindex].t[1]);
        printf("Pressure at site altitude    (%12f m) = %8f atm\n", atm->SiteAlt, atm->vprof_pressure.val[zindex]);
        ...
        if (atm->SiteTPauto == 0)
        {
            ...
            for (int k = 0; k < 10; k++)
            {
                ...
                printf("h = %.0f m   Temp = %f  (%f)  Press = %f  (%f)\n", h, output[zindex].t[1], atm->SiteTemp,
                       atm->vprof_pressure.val[zindex], atm->SitePress);
    ...
    TotPart0 = (double *)malloc(sizeof(double) * ATM_VPROF_NBSTEP); // total number of particles per cm3
    ...
    for (long zindex = 0; zindex < ATM_VPROF_NBSTEP; zindex++)
    {
        ...
        fprintf(fp, "%6.0f %12f %12f %12f %12f %12f %12f %12f %.8g %5f\n", input[zindex].alt * 1000.0,
                output[zindex].d[2], output[zindex].d[3], output[zindex].d[4], output[zindex].d[6], output[zindex].d[0],
                output[zindex].d[1] + output[zindex].d[8], output[zindex].d[7], output[zindex].d[5],
                output[zindex].t[1]);
        ...
        atm->vprof_dens_species[speciesNe].val[zindex] = output[zindex].d[2] * 2.33e-5;
        TotPart0[zindex] += atm->vprof_dens_species[speciesNe].val[zindex]; // total density

        atm->vprof_dens_species[speciesCH4].val[zindex] = TotPart0[zindex] * 2e-6;
        if (h < 45000.0)
        {
            atm->vprof_dens_species[speciesCH4].val[zindex] *= 1.0 - h / 45000.0;
        }
        else
        {
            atm->vprof_dens_species[speciesCH4].val[zindex] = 0.0;
        }
        TotPart0[zindex] += atm->vprof_dens_species[speciesCH4].val[zindex]; // total density

        atm->vprof_dens_species[speciesKr].val[zindex] = output[zindex].d[2] * 1.46e-6;
        TotPart0[zindex] += atm->vprof_dens_species[speciesKr].val[zindex]; // total density

        atm->vprof_dens_species[speciesH2].val[zindex] = output[zindex].d[2] * 7.04e-7;
        TotPart0[zindex] += atm->vprof_dens_species[speciesH2].val[zindex]; // total density

        atm->vprof_dens_species[speciesO3].val[zindex] =
            300.0 * 2.69e20 / (4250.0 * sqrt(2.0 * M_PI)) * exp(-0.5 * pow(((h - 25000.0) / 4250.0), 2.0)); // [m-3]
        atm->vprof_dens_species[speciesO3].val[zindex] *= 1.0e-6;

        if (h < 70000)
        {
            atm->vprof_dens_species[speciesCO2].val[zindex] = atm->CO2_ppm * 1e-6 * TotPart0[zindex];
        }
        else
        {
        ...
        //    dens0 = TotPart0[i];
        //}
        TotPart0[zindex] *= 1000.0; // convert from cm-3 to cm-2   each bin is 10m = 1000cm
        ...
        if (h > atm->SiteAlt)
        {
            ...
            TotPart1 += 2.5e-6 * TotPart0[zindex] * exp(-3.0 * pow(h / 100000.0, 4.0));

            // exponential mixing term (not yet scaled)
            TotPart2 += (exp(-h / atm->SitePWSH)) * TotPart0[zindex] * exp(-3.0 * pow(h / 100000.0, 4.0));
        }

        // for now, assume fixed background mixing ratio
        atm->vprof_dens_species[speciesH2O].val[zindex] = 2.5e-6 * TotPart0[zindex] / 1000.0; // cm-3
    }
    fclose(fp);

    printf("TOTAL particles : %g part.cm^-2\n", TotPart);
    printf("TOTAL particles H2O fixed mixing ratio : %g part.cm^-2    ( SitePWSH = %f, SiteAlt = %f m)\n", TotPart1,
           atm->SitePWSH, atm->SiteAlt);
    ...
    if (atm->SiteH2OMethod == 1)
    {
        ...
        for (long zindex = 0; zindex < ATM_VPROF_NBSTEP; zindex++)
        {
            ...
            atm->vprof_dens_species[speciesH2O].val[zindex] += atm->alpha1H2O * (exp(-h / atm->SitePWSH)) *
    ...
    if (atm->SiteH2OMethod == 2)
    {
        ...
        densH20_site = (atm->SiteRH / 100.0) * LoschmidtConstant * 1e-6 * H2OSatTempSite / 101325.0; // cm3
        ...
        for (long zindex = 0; zindex < ATM_VPROF_NBSTEP; zindex++)
        {
            double h = ATM_VPROF_STEPSIZE * zindex;
            //            densH2O[i] /= 1000.0;
            atm->vprof_dens_species[speciesH2O].val[zindex] += densH20_site * exp(-(h - atm->SiteAlt) / atm->SitePWSH) *
                                                               (TotPart0[zindex] / TotPart0[i0]) *
                                                               exp(-3.0 * pow(h / 100000.0, 4.0));
            if (h > atm->SiteAlt)
            {
                TotPart2 += 100.0 * densH20_site * exp(-(h - atm->SiteAlt) / atm->SitePWSH) *
                            exp(-3.0 * pow(h / 100000.0, 4.0));
            }
        }
    ...
    if (atm->SiteH2OMethod == 3)
    {
...

        {
            ...
            for (int iter = 0; iter < 15; iter++)
            {
                ...
                for (long zindex = 0; zindex < ATM_VPROF_NBSTEP; zindex++)
                {
                    ...
                    if (h > atm->SiteAlt)
                    {
                        TotPart2 += 1000.0 * densH20_site * exp(-(h - atm->SiteAlt) / pwsh) *
                                    exp(-3.0 * pow(h / 100000.0, 4.0));
                    }
                }
        ...
        for (long zindex = 0; zindex < ATM_VPROF_NBSTEP; zindex++)
        {
            double h = ATM_VPROF_STEPSIZE * zindex;
            //          densH2O[i] /= 1000.0;
            atm->vprof_dens_species[speciesH2O].val[zindex] += densH20_site * exp(-(h - atm->SiteAlt) / pwsh) *
                                                               (TotPart0[zindex] / TotPart0[i0]) *
                                                               exp(-3.0 * pow(h / 100000.0, 4.0));
        }
    }
...

    {
        ...
        for (long zindex = 0; zindex < ATM_VPROF_NBSTEP; zindex++)
        {
            ...
            if (h > atm->SiteAlt)
            {
                tpw += atm->vprof_dens_species[speciesH2O].val[zindex] * 1000.0; // particules per cm2
            }
        }
        printf("    H2O :   %g particle/cm2\n", tpw);
        tpw /= 6.0221413e23;
...

    {
        ...
        printf("    RH  :   %f %%\n", atm->vprof_dens_species[speciesH2O].val[i0] / LoschmidtConstant * 1e6 /
                                          H2OSatTempSite * 101325.0 * 100.0);
    }
    ...
    for (long zindex = 0; zindex < ATM_VPROF_NBSTEP; zindex++)
    {
        ...
        if (h > atm->SiteAlt)
        {
            TotPart1 += atm->vprof_dens_species[speciesH2O].val[zindex] * 1000.0;
        }

        // total particule densisty without H2O and O3
        atm->vprof_denstot.val[zindex] =
            output[zindex].d[0] + output[zindex].d[1] + output[zindex].d[2] + output[zindex].d[3] +
            output[zindex].d[4] + output[zindex].d[6] + output[zindex].d[7] + output[zindex].d[8] +
            atm->vprof_dens_species[speciesCO2].val[zindex] + atm->vprof_dens_species[speciesNe].val[zindex];

        double coeff = (atm->vprof_denstot.val[zindex] + atm->vprof_dens_species[speciesH2O].val[zindex] +
                        atm->vprof_dens_species[speciesO3].val[zindex]) /
                       atm->vprof_denstot.val[zindex];
        ...
        atm->vprof_RH.val[zindex] = (atm->vprof_dens_species[speciesH2O].val[zindex] * 1e6 / LoschmidtConstant) *
                                    101325.0 / AtmosphereModel_H2O_Saturation(atm->vprof_temperature.val[zindex]);
